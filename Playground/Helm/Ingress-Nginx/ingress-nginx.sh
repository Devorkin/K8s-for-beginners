...
if ! which helm &> /dev/null; then echo -e "`date +"%d-%m-%y %H:%M:%S"`\tHelm is missing!" | tee -a /var/log/ingress-nginx.log; exit 2; fi
...
if [ ! $nodes_count -gt 0 ]; then
  echo -e "`date +"%d-%m-%y %H:%M:%S"`\tIngress-Nginx setup requires at least 1 node, currently there are ${nodes_count} registered, please enlarge your K8s cluster" | tee -a /var/log/ingress-nginx.log
  exit 3
fi
...
install() {
  ...
  if [ ! -f /var/run/ingress-nginx.pid ]; then
    ...
    echo -e "`date +"%d-%m-%y %H:%M:%S"`\tIngress-Nginx setup is already running in the background..." | tee -a /var/log/ingress-nginx.log
    echo -e "`date +"%d-%m-%y %H:%M:%S"`\tUse \`kubectl get events -n ingress-nginx -w\` to check the setup progress..." | tee -a /var/log/ingress-nginx.log
    exit 4
  fi
